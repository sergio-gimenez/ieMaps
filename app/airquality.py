...
def getstation(lat_start, lon_start, lat_end, lon_end):
    ...
    stations_path = "./Datasets/Qualitat_Aire_Estacions.csv"

    stations_path = wget.download(stations_url)
    ...
    for station in range(0,len(stations)):
        g = geodesic((m['lat2'], m['lon2']), (stations.iloc[station]['Latitud'], stations.iloc[station]['Longitud'])).km
        if g < min_g:
            min_g = g
            closest_station_lon = stations.iloc[station]['Longitud']

    closest_station = stations.loc[stations['Longitud']== closest_station_lon]['Estacio']
